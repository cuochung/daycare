---
name: api-usage
description: Daycare 專案的 API 使用規範和方法。定義如何使用 api.js 模組進行資料庫操作、檔案上傳等。當需要進行資料庫查詢、新增、更新、刪除操作，或呼叫後端 API 時使用。
---

# daycare API 使用規範

本 skill 定義了 daycare 專案中 API 模組的使用方法和最佳實踐。
...
專案使用的資料庫名稱為 `daycare`，已透過 Pinia store 統一管理。
...
- `api.get('personnel')` → `general/getAll/daycare/personnel`
- `api.add('scheduling', data)` → `general/add/daycare/scheduling`
...
- `store.state.databaseName` - 資料庫名稱（預設：daycare）
