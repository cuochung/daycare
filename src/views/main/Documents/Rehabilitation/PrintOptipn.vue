<template>
  <v-dialog v-model="dialog" max-width="600px" persistent :retain-focus="false">
    <v-card rounded="xl">
      <v-card-title class="d-flex align-center bg-primary-lighten-2">
        <v-icon color="primary" class="mr-2">mdi-printer</v-icon>
        <span class="text-h6 font-weight-bold">列印復健評估表設定</span>
        <v-spacer />
...
const handlePrintRange = async () => {
  ...
  if (result?.isConfirmed) {
    ...
    const filtered = items.value.filter((item) => {
      const itemDate = dayjs(item.date || item.raw?.date).format('YYYY-MM-DD')
