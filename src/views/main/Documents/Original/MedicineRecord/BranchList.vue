          ...
          <v-card class="my-2" v-show="!editMode" style="overflow:auto" rounded="lg" variant="outlined">
            <v-card-text>
              <v-sheet color="primary" variant="tonal" class="text-center pa-2 mb-3" rounded="md">
                <h3 class="text-subtitle-1 font-weight-medium">已建構科別列表(點選可修改)</h3>
              </v-sheet>
...
const headerTitle = computed(() => (editMode.value ? '修改科別資料' : '科別資料管理'))
const headerSubtitle = computed(() => (editMode.value ? '更新科別資訊' : '管理科別列表'))
...
const addOK = async () => {
  ...
  try {
    ...
    const response = await api.add('branch', postData)
    if (response?.state == 1) {
      store.showToastMulti({
        type: 'success',
        message: '已新增',
...
const del = async (delData) => {
  ...
  if (result.isConfirmed) {
    ...
    try {
      const response = await api.delete('branch', postData)
      if (response?.state == 1) {
        store.showToastMulti({
          type: 'success',
          message: '已刪除',
...
const editOK = async () => {
  try {
    ...
    const response = await api.post('branch', postData)
    if (response?.state == 1) {
      store.showToastMulti({
        type: 'success',
        message: '已修改',
