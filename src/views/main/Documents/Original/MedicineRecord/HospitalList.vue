        ...
        <v-sheet :color="headerColor" class="d-flex align-center justify-space-between px-6 py-4" rounded="t-xl">
          <div class="d-flex align-center">
            <v-avatar size="48" color="white" variant="tonal" class="mr-4">
              <v-icon size="28">mdi-hospital-building</v-icon>
            </v-avatar>
            <div>
          ...
          <v-card class="my-2" v-show="!editMode" style="overflow:auto" rounded="lg" variant="outlined">
            <v-card-text>
              <v-sheet color="primary" variant="tonal" class="text-center pa-2 mb-3" rounded="md">
                <h3 class="text-subtitle-1 font-weight-medium">已建構醫院列表(點選可修改)</h3>
              </v-sheet>
...
const headerTitle = computed(() => (editMode.value ? '修改醫院資料' : '醫院資料管理'))
const headerSubtitle = computed(() => (editMode.value ? '更新醫院資訊' : '管理醫院列表'))
...
const addOK = async () => {
  ...
  try {
    ...
    const response = await api.add('hospital', postData)
    if (response?.state == 1) {
      store.showToastMulti({
        type: 'success',
        message: '已新增',
...
const del = async (delData) => {
  ...
  if (result.isConfirmed) {
    ...
    try {
      const response = await api.delete('hospital', postData)
      if (response?.state == 1) {
        store.showToastMulti({
          type: 'success',
          message: '已刪除',
...
const editOK = async () => {
  try {
    ...
    const response = await api.post('hospital', postData)
    if (response?.state == 1) {
      store.showToastMulti({
        type: 'success',
        message: '已修改',
