<template>
  <div class="maincauseoftreatmentlist">
          ...
          <v-card class="my-2" v-show="!editMode" style="overflow:auto" rounded="lg" variant="outlined">
            <v-card-text>
              <v-sheet color="primary" variant="tonal" class="text-center pa-2 mb-3" rounded="md">
                <h3 class="text-subtitle-1 font-weight-medium">已建構就診主因列表(點選可修改)</h3>
              </v-sheet>
...
const getAllData = async () => {
  try {
    const response = await api.get('main_cause_of_treatment')
...
const headerTitle = computed(() => (editMode.value ? '修改就診主因資料' : '就診主因管理'))
const headerSubtitle = computed(() => (editMode.value ? '更新就診主因資訊' : '管理就診主因列表'))
const headerColor = computed(() => (editMode.value ? 'success' : 'primary'))
...
const addOK = async () => {
  ...
  try {
    ...
    const response = await api.add('main_cause_of_treatment', postData)
    if (response?.state == 1) {
      store.showToastMulti({
        type: 'success',
        message: '已新增',
...
const del = async (delData) => {
  ...
  if (result.isConfirmed) {
    ...
    const postData = {
      snkey: delData.snkey,
      tablename: 'main_cause_of_treatment',
          info: JSON.stringify(delData),
    }

    try {
      const response = await api.delete('main_cause_of_treatment', postData)
      if (response?.state == 1) {
        store.showToastMulti({
          type: 'success',
          message: '已刪除',
...
const editOK = async () => {
  try {
    ...
    const response = await api.post('main_cause_of_treatment', postData)
    if (response?.state == 1) {
      store.showToastMulti({
        type: 'success',
        message: '已修改',
