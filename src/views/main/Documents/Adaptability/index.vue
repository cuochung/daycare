<template>
  <div class="adaptability">
    <AdaptabilityAddDialog ref="addDialogRef" @refresh="fetchRecords" />

    <v-container fluid class="pa-0">
        <v-row>
        <v-col cols="12">
          <v-sheet class="adaptability__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                <v-avatar size="56" color="primary" variant="tonal" class="mr-4">
                  <v-icon color="primary" size="32">mdi-account-check</v-icon>
                </v-avatar>
                <div>
                  <h2 class="adaptability__title mb-1">適應性評估表</h2>
                  <p class="adaptability__subtitle mb-0">
            ...
            <div class="adaptability__meta d-flex flex-wrap ga-3">
      ...
      <v-row class="mt-4" dense>
        <v-col cols="12" md="4">
          <v-card variant="tonal" color="primary" rounded="xl" class="adaptability__summary-card">
          ...
          <v-col cols="12" md="4">
          <v-card variant="tonal" color="info" rounded="xl" class="adaptability__summary-card">
      ...
      <v-row class="mt-4">
        <v-col cols="12">
          <v-sheet class="adaptability__toolbar" rounded="xl" elevation="0">
        ...
        <v-col cols="12">
          ...
          <PaginatedIterator v-if="filteredCount" :items="filteredItems" v-model:items-per-page="itemsPerPage"
            ...
            <template #default="{ items }">
              <v-card class="adaptability__table-card" rounded="xl" variant="outlined">
                ...
                <v-card-text class="pa-0">
                  <v-table class="adaptability__table text-no-wrap" fixed-header>
                    <thead>
                      <tr>
                        <th style="width: 80px;" class="text-center">操作</th>
                        <th style="width: 120px;">評估日期</th>
                        <th style="width: 140px;">1.入住意願接受度</th>
                        <th style="width: 120px;">2.家屬配合度</th>
                        <th style="width: 100px;">3.情緒</th>
                        <th style="width: 100px;">4.行為</th>
                        <th style="width: 140px;">5.社會化(參與度)</th>
                        <th style="width: 120px;">6.家庭支持度</th>
                        <th style="width: 80px;">總分</th>
                        <th style="width: 200px;">結果說明</th>
                    ...
                    <tbody>
                      <tr v-for="(record, index) in items"
                        :key="record.snkey || record.raw?.snkey || `adaptability-${index}`">
                        ...
                        <td>{{ getQuestionText(record.raw?.question1) }}</td>
                        <td>{{ getQuestionText(record.raw?.question2) }}</td>
                        <td>{{ getQuestionText(record.raw?.question3) }}</td>
                        <td>{{ getQuestionText(record.raw?.question4) }}</td>
                        <td>{{ getQuestionText(record.raw?.question5) }}</td>
                        <td>{{ getQuestionText(record.raw?.question6) }}</td>
                        <td class="text-center font-weight-bold">{{ record.raw?.total || '—' }}</td>
                        <td class="text-truncate" style="max-width: 200px;">{{ record.raw?.result_description || '—' }}
                        </td>
                        ...
                        <td v-if="canShowEditorInfo">
                          <div class="text-body-2 text-truncate adaptability__edit-info">
                            {{
...
import AdaptabilityAddDialog from './Add.vue'
...
const fetchRecords = async () => {
  ...
  const url = `byjson/searchByKeyValue/${store.state.databaseName}/adaptability`
...
const openAddDialog = () => {
  if (!hasUser.value) {
    proxy?.$swal?.({ icon: 'warning', title: '請先選擇住民，再新增適應性評估資料。' })
    return
  }
...
const askDelete = (record) => {
  proxy.$swal({
    ...
    const response = await api.delete('adaptability', payload)
    if (response?.state == 1) {
