<template>
  <div class="pressure-injuries-list">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="pressure-injuries-list__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                <v-avatar size="56" color="primary" variant="tonal" class="mr-4">
                  <v-icon color="primary" size="32">mdi-shield-alert-outline</v-icon>
                </v-avatar>
                <div>
                  <h2 class="pressure-injuries-list__title mb-1">壓力性損傷危險評估</h2>
                  <p class="pressure-injuries-list__subtitle mb-0">
                    評估住民壓力性損傷風險，包含知覺感受、潮溼程度、活動能力、移動能力、營養攝取、摩擦力/剪力等六項評估。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="pressure-injuries-list__meta d-flex flex-wrap ga-3">
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex align-center">
              ...
              <PressureInjuriesAdd ref="addDialogRef" :useDataBase="useDataBase" @refresh="getAllData" />
            </v-card-title>
            <v-card-text>
                  ...
                  <v-table fixed-header class="text-no-wrap">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in items" :key="index">
                        ...
                        <td>{{ item.raw.Date }}</td>
                        <td>{{ calculateTotalScore(item.raw) }}</td>
...
import PressureInjuriesAdd from './Add.vue'
import mohwFn from '@/assets/js/mohwFn'

const store = useStore()
const { proxy } = getCurrentInstance()

const useDataBase = 'pressureinjuries'
const items = ref([])
const searchKey = ref('')
...
const dataToJson = (postItem) => {
  ...
  const finalPostData = mohwFn.returnPressureInjuriesJSON(postData)
  const finalObj = {
    CaseID: store.state.uData.id_num,
    EndDate: store.state.uData.in_date,
    ...store.state.mohwAllManArrs,
  }
  finalObj['PressureInjuries'] = finalPostData
...
const calculateTotalScore = (item) => {
  if (!item) return 0
  let total = 0
  const questions = ['qq1', 'qq2', 'qq3', 'qq4', 'qq5', 'qq6']

  questions.forEach((q) => {
    if (item[q] && item[q].Point !== undefined) {
      total += item[q].Point
    }
  })
