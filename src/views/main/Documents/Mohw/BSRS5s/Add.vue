      ...
      <v-card-text class="px-6 py-5">
        <v-form ref="formRef">
          ...
          <v-sheet v-if="isUnableToAssess" class="mb-6 pa-4" rounded="lg" color="warning-lighten-5" elevation="1"
            ...
            <v-text-field label="請說明無法評估的原因" v-model="list.remark" variant="outlined" density="comfortable"
          ...
          <div v-if="!isUnableToAssess">
            <v-sheet class="mb-4 pa-4" rounded="lg" color="blue-lighten-5" elevation="1" border>
              <v-row>
                <v-col cols="12" md="6">
                  <v-select :items="optionItems" label="1. 睡眠困難，譬如難以入睡、易醒或早醒" v-model="list.qq1" :rules="emptyRules"
                    return-object item-title="text" item-value="text" variant="outlined"
                    density="comfortable"></v-select>
                </v-col>
                <v-col cols="12" md="6">
                  <v-select :items="optionItems" label="2. 感覺緊張或不安" v-model="list.qq2" :rules="emptyRules" return-object
                    item-title="text" item-value="text" variant="outlined" density="comfortable"></v-select>
                </v-col>
              </v-row>
            </v-sheet>
            ...
            <v-sheet class="mb-4 pa-4" rounded="lg" color="green-lighten-5" elevation="1" border>
              <v-row>
                <v-col cols="12" md="6">
                  <v-select :items="optionItems" label="3. 覺得容易苦惱或動怒" v-model="list.qq3" :rules="emptyRules"
                    return-object item-title="text" item-value="text" variant="outlined"
                    density="comfortable"></v-select>
                </v-col>
                <v-col cols="12" md="6">
                  <v-select :items="optionItems" label="4. 感覺憂鬱、心情低落" v-model="list.qq4" :rules="emptyRules"
                    return-object item-title="text" item-value="text" variant="outlined"
                    density="comfortable"></v-select>
                </v-col>
              </v-row>
            ...
            <v-sheet class="mb-4 pa-4" rounded="lg" color="orange-lighten-5" elevation="1" border>
              <v-row>
                <v-col cols="12" md="6">
                  <v-select :items="optionItems" label="5. 覺得比不上別人" v-model="list.qq5" :rules="emptyRules" return-object
                    item-title="text" item-value="text" variant="outlined" density="comfortable"></v-select>
                </v-col>
                <v-col cols="12" md="6">
                  <v-select :items="optionItems" label="6. 有自殺的想法" v-model="list.qq6" :rules="emptyRules" return-object
                    item-title="text" item-value="text" variant="outlined" density="comfortable"></v-select>
                </v-col>
              </v-row>
            </v-sheet>
...
const headerTitle = computed(() => (processType.value === 'add' ? '新增簡式健康量表評估' : '修改簡式健康量表評估'))
const headerSubtitle = computed(() =>
  processType.value === 'add'
    ? '填寫住民心理健康狀況評估，包含睡眠、緊張、憂鬱等六項評估項目。'
...
const openForAdd = () => {
  ...
  if (props.items && props.items.length > 0) {
    ...
    // 根據 remark 是否存在來設置 checkbox 狀態
    isUnableToAssess.value = !!list.remark
  }
