<template>
  <div class="case-summary-plan-list">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="case-summary-plan-list__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                ...
                <div>
                  <h2 class="case-summary-plan-list__title mb-1">需求摘要&照護計畫</h2>
                  <p class="case-summary-plan-list__subtitle mb-0">
                    評估住民需求摘要並制定照護計畫，包含問題評估、目標設定、措施執行等。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="case-summary-plan-list__meta d-flex flex-wrap ga-3">
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex align-center">
              ...
              <CaseSummaryPlanAdd ref="addDialogRef" :useDataBase="useDataBase" @refresh="getAllData" />
            </v-card-title>
            <v-card-text>
...
import CaseSummaryPlanAdd from './Add.vue'
import mohwFn from '@/assets/js/mohwFn'

const store = useStore()
const { proxy } = getCurrentInstance()

const useDataBase = 'casesummaryplan'
const items = ref([])
const searchKey = ref('')
...
const pushToMOHW = async (postItem) => {
  ...
  if (result.isConfirmed) {
    try {
      ...
      */
      let jsonfile = JSON.stringify(dataToJson_CaseSummary(postItem))
      await mohwFn.pushToMOHW(postItem, useDataBase, jsonfile, 'CaseSummary')

      jsonfile = JSON.stringify(dataToJson_CarePlan(postItem))
      const result = await mohwFn.pushToMOHW(postItem, useDataBase, jsonfile, 'CarePlan')
      if (result) {
        await getAllData()
      }
    } catch (error) {
...
const checkMOHWData = async (postItem) => {
  try {
    const result = await mohwFn.checkMOHWData(postItem, useDataBase, 'CarePlan')
    if (result) {
      await getAllData()
    }
  } catch (error) {
...
const dataToJson_CaseSummary = (postItem) => {
  const postData = [postItem]
  const finalPostData = mohwFn.returnCaseSummaryJSON(postData, store.state.uData)
...
const dataToJson_CarePlan = (postItem) => {
  const postData = [postItem]
  const finalPostData = mohwFn.returnCarePlanJSON(postData, store.state.uData)
