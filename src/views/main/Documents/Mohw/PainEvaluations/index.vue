<template>
  <div class="pain-evaluations-list">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="pain-evaluations-list__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                ...
                <div>
                  <h2 class="pain-evaluations-list__title mb-1">疼痛評估</h2>
                  <p class="pain-evaluations-list__subtitle mb-0">
                    評估住民疼痛狀況，包含部位、分數、時間、性質等評估項目。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="pain-evaluations-list__meta d-flex flex-wrap ga-3">
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex align-center">
              ...
              <PainEvaluationsAdd ref="addDialogRef" :useDataBase="useDataBase" @refresh="getAllData" />
            </v-card-title>
            <v-card-text>
...
import PainEvaluationsAdd from './Add.vue'
import mohwFn from '@/assets/js/mohwFn'

const store = useStore()
const { proxy } = getCurrentInstance()

const useDataBase = 'painevaluations'
const items = ref([])
const searchKey = ref('')
...
const dataToJson = (postItem) => {
  ...
  const finalPostData = mohwFn.returnPainEvaluationsJSON(postData)
  const finalObj = {
    CaseID: store.state.uData.id_num,
    EndDate: store.state.uData.in_date,
    ...store.state.mohwAllManArrs,
  }
  finalObj['PainEvaluations'] = finalPostData
