      ...
      <v-card>
        ...
        <v-card-text>
          <v-form ref="form" class="mt-3">
            ...
            <v-text-field label="備註(無法評估時請備註原因)" v-model="list.note"></v-text-field>
            <div v-if="!list.note">
              <v-autocomplete :items="YnItems" label="您對您的生活感到滿意嗎？ " v-model="list.qq1" :rules="emptyRules"
                return-object></v-autocomplete>
              <v-autocomplete :items="YnItems2" label="您是否常常感到厭煩？" v-model="list.qq2" :rules="emptyRules"
                return-object></v-autocomplete>
              <v-autocomplete :items="YnItems" label="您是否常常感到無論做什麼都沒有用？" v-model="list.qq3" :rules="emptyRules"
                return-object></v-autocomplete>
              <v-autocomplete :items="YnItems" label="您是否比較喜歡待在家裡，較不喜歡外出及不喜歡做新的事？" v-model="list.qq4"
                :rules="emptyRules" return-object></v-autocomplete>
              <v-autocomplete :items="YnItems" label="您是否感覺活得很沒有價值？" v-model="list.qq5" :rules="emptyRules"
                return-object></v-autocomplete>
            </div>
