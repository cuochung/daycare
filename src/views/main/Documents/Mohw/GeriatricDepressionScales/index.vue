<template>
  <div class="geriatric-depression-scales-list">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="geriatric-depression-scales-list__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                <v-avatar size="56" color="primary" variant="tonal" class="mr-4">
                  <v-icon color="primary" size="32">mdi-emoticon-sad</v-icon>
                </v-avatar>
                <div>
                  <h2 class="geriatric-depression-scales-list__title mb-1">情緒問題評估</h2>
                  <p class="geriatric-depression-scales-list__subtitle mb-0">
                    評估住民情緒問題狀況，包含憂鬱、焦慮等評估項目。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="geriatric-depression-scales-list__meta d-flex flex-wrap ga-3">
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex align-center">
              ...
              <GeriatricDepressionScalesAdd ref="addDialogRef" :useDataBase="useDataBase" @refresh="getAllData" />
            </v-card-title>
            <v-card-text>
...
import GeriatricDepressionScalesAdd from './Add.vue'
import mohwFn from '@/assets/js/mohwFn'

const store = useStore()
const { proxy } = getCurrentInstance()

const useDataBase = 'geriatricdepressionscales'
const items = ref([])
const searchKey = ref('')
...
const dataToJson = (postItem) => {
  ...
  const finalPostData = mohwFn.returnGeriatricDepressionScalesJSON(postData)
  const finalObj = {
    CaseID: store.state.uData.id_num,
    EndDate: store.state.uData.in_date,
    ...store.state.mohwAllManArrs,
  }
  finalObj['GeriatricDepressionScales'] = finalPostData
...
.geriatric-depression-scales-list {
  &__hero {
    padding: 28px;
