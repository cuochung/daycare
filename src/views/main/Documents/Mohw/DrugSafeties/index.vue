<template>
  <div class="drug-safeties-list">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="drug-safeties-list__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                ...
                <div>
                  <h2 class="drug-safeties-list__title mb-1">藥物安全性評估表</h2>
                  <p class="drug-safeties-list__subtitle mb-0">
                    評估住民藥物安全性狀況，包含多重用藥、不當用藥等評估項目。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="drug-safeties-list__meta d-flex flex-wrap ga-3">
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex align-center">
              ...
              <DrugSafetiesAdd ref="addDialogRef" :items="items" @refresh="getAllData" />
            </v-card-title>
            <v-card-text>
...
import DrugSafetiesAdd from './Add.vue'
import mohwFn from '@/assets/js/mohwFn'

const store = useStore()
const { proxy } = getCurrentInstance()

const useDataBase = 'drugsafeties'
const items = ref([])
const searchKey = ref('')
...
const dataToJson = (postItem) => {
  ...
  const finalPostData = mohwFn.returnDrugSafetiesJSON(postData)
  const finalObj = {
    CaseID: store.state.uData.id_num,
    EndDate: store.state.uData.in_date,
    ...store.state.mohwAllManArrs,
  }
  finalObj['DrugSafeties'] = finalPostData
