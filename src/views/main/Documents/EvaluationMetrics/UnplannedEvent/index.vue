<template>
  <div class="unplanned-event-list">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="unplanned-event-list__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                ...
                <div>
                  <h2 class="unplanned-event-list__title mb-1">非計畫性轉急性住院登記表</h2>
                  <p class="unplanned-event-list__subtitle mb-0">
                    記錄住民非計畫性轉至急性醫院住院事件，包含登記日期、住院原因等評估項目。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="unplanned-event-list__meta d-flex flex-wrap ga-3">
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex align-center">
              ...
              <UnplannedEventAdd ref="addDialogRef" :useDataBase="useDataBase" :items="items" @refresh="getAllData"
                  ...
                  <v-table fixed-header class="text-no-wrap">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in items" :key="index">
                        ...
                        <td class="text-center">
                          <v-icon v-if="item.raw.livein72hour" color="success">mdi-check-circle</v-icon>
                          <span v-else>—</span>
                        </td>
...
import UnplannedEventAdd from './Add.vue'
import PaginatedIterator from '@/components/PaginatedIterator.vue'

const store = useStore()
const { proxy } = getCurrentInstance()

const useDataBase = ref('unplannedevent')
const items = ref([])
const searchKey = ref('')
...
const columnCount = computed(() => {
  let count = 4 // 功能、登記日期、非計畫性轉至急性醫院住院、入住後72小時內
