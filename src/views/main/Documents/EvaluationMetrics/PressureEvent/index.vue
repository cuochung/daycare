<template>
  <div class="pressure-event-list">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="pressure-event-list__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                <v-avatar size="56" color="primary" variant="tonal" class="mr-4">
                  <v-icon color="primary" size="32">mdi-alert-octagon</v-icon>
                </v-avatar>
                <div>
                  <h2 class="pressure-event-list__title mb-1">壓力性損傷登記表</h2>
                  <p class="pressure-event-list__subtitle mb-0">
                    記錄住民壓力性損傷事件，包含發現日期、部位、級數、範圍等評估項目。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="pressure-event-list__meta d-flex flex-wrap ga-3">
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex align-center">
              ...
              <PressureEventAdd ref="addDialogRef" :useDataBase="useDataBase" :items="items" @refresh="getAllData"
                  ...
                  <v-table fixed-header class="text-no-wrap">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in items" :key="index">
                        ...
                        <td>{{ item.raw.Date || '—' }}</td>
                        <td>{{ item.raw.createInfo?.name || '—' }}</td>
