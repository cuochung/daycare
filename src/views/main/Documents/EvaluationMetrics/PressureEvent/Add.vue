      ...
      <v-sheet :color="headerColor" class="d-flex align-center justify-space-between px-6 py-4" rounded="t-xl">
        <div class="d-flex align-center">
          <v-avatar size="48" color="white" variant="tonal" class="mr-4">
            <v-icon color="white" size="28">mdi-alert-octagon</v-icon>
          </v-avatar>
          <div>
      ...
      <v-card-text class="px-6 py-5">
        <v-form ref="formRef">
          ...
          <v-sheet class="mb-6 pa-4" rounded="lg" color="primary-lighten-5" elevation="1" border>
            ...
            <v-row>
              <v-col cols="12" md="4">
                <v-text-field v-model="list.adl" label="巴氏量表" variant="outlined" density="comfortable"
                  prepend-inner-icon="mdi-numeric" :rules="emptyRules" />
              </v-col>
              <v-col cols="12" md="4">
                <v-text-field v-model="list.Date" type="date" label="壓力性損傷發現日期" variant="outlined"
                  density="comfortable" prepend-inner-icon="mdi-calendar" :rules="emptyRules" />
              </v-col>
              <v-col cols="12" md="4">
                <v-text-field v-model="list.part" label="壓力性損傷部位" variant="outlined" density="comfortable"
                  prepend-inner-icon="mdi-map-marker" :rules="emptyRules" />
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12" md="4">
                <v-autocomplete v-model="list.hurtlv" :items="hurtItems" label="級數" variant="outlined"
                  density="comfortable" :rules="emptyRules" />
              </v-col>
              <v-col cols="12" md="4">
                <v-text-field v-model="list.cm" label="範圍(CM)" variant="outlined" density="comfortable"
                  prepend-inner-icon="mdi-ruler" :rules="emptyRules" />
              </v-col>
              <v-col cols="12" md="4">
                <v-text-field v-model="list.OKDate" type="date" label="癒合日期" variant="outlined"
...
const hurtItems = ['1級', '2級', '3級', '4級', '無法分級', '深部組織', '黏膜層']
...
const headerTitle = computed(() => (processType.value === 'add' ? '新增壓力性損傷登記' : '修改壓力性損傷登記'))
const headerSubtitle = computed(() =>
  processType.value === 'add'
    ? '填寫住民壓力性損傷事件，包含發現日期、部位、級數、範圍等評估項目。'
    : '更新既有壓力性損傷登記紀錄，維持數據正確性與一致性。'
)
