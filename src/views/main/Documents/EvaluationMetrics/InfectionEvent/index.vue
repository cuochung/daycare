<template>
  <div class="infection-event-list">
    <v-container fluid class="pa-0">
      <v-row>
        <v-col cols="12">
          <v-sheet class="infection-event-list__hero" rounded="xl" elevation="0">
            <div class="d-flex flex-column flex-md-row justify-space-between align-start">
              <div class="d-flex align-center mb-4 mb-md-0">
                <v-avatar size="56" color="primary" variant="tonal" class="mr-4">
                  <v-icon color="primary" size="32">mdi-virus</v-icon>
                </v-avatar>
                <div>
                  <h2 class="infection-event-list__title mb-1">感染登記表</h2>
                  <p class="infection-event-list__subtitle mb-0">
                    記錄住民感染事件，包含感染日期、部位、康復日期等評估項目。
                  </p>
                </div>
              </div>
            </div>
            ...
            <div class="infection-event-list__meta d-flex flex-wrap ga-3">
        ...
        <v-col cols="12">
          <v-card>
            <v-card-title class="d-flex align-center">
              ...
              <InfectionEventAdd ref="addDialogRef" :useDataBase="useDataBase" :items="items" @refresh="getAllData"
...
import InfectionEventAdd from './Add.vue'
import PaginatedIterator from '@/components/PaginatedIterator.vue'

const store = useStore()
const { proxy } = getCurrentInstance()

const useDataBase = ref('infectionevent')
const items = ref([])
const searchKey = ref('')
