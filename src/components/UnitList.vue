          ...
          <v-card class="my-2" v-show="!editMode" style="overflow:auto" rounded="lg" variant="outlined">
            <v-card-text>
              <v-sheet color="primary" variant="tonal" class="text-center pa-2 mb-3" rounded="md">
                <h3 class="text-subtitle-1 font-weight-medium">已建構單位列表(點選可修改)</h3>
              </v-sheet>
...
const addOK = async () => {
  ...
  try {
    ...
    const response = await api.add('unit', postData)
    if (response?.state == 1) {
      store.showToastMulti({
        type: 'success',
        message: '已新增',
...
const del = async (delData) => {
  ...
  if (result.isConfirmed) {
    ...
    try {
      const response = await api.delete('unit', postData)
      if (response?.state == 1) {
        store.showToastMulti({
          type: 'success',
          message: '已刪除',
...
const editOK = async () => {
  try {
    ...
    const response = await api.post('unit', postData)
    if (response?.state == 1) {
      store.showToastMulti({
        type: 'success',
        message: '已修改',
